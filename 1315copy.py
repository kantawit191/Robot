...
def load_high_low():
    if os.path.exists('high_low_status.json'):
        ...
        with open('high_low_status.json', 'w') as file:
            json.dump({}, file)  # สร้างไฟล์ใหม่พร้อมข้อมูลว่าง
        cmd_notify("สร้างไฟล์ high_low_status.json สำเร็จ")
        return {}
...
def place_buy_order(symbol, buy_usdt_amount, lowest_price, trade_status):
    ...
    with threading.Lock():
        try:
            ...
            # ตรวจสอบราคาปัจจุบันกับ target_buy_price และสั่งซื้อทันทีหากราคาถึงเป้าหมาย
            cmd_notify(f"กำลังตรวจสอบราคาปัจจุบัน {current_price} กับเป้าหมายการซื้อ {target_buy_price}")
            if current_price >= target_buy_price:
                cmd_notify(f"{symbol}: ราคาถึงหรือเกินเป้าหมายการซื้อที่ {current_price} สั่งซื้อทันที")
            else:
                cmd_notify(f"ไม่ซื้อ {symbol}: ราคาปัจจุบัน {current_price} ต่ำกว่าเป้าหมาย {target_buy_price}")
                return None, None
...
def handle_socket_message(msg, trade_status, buy_usdt_amount, target_prices, highest_prices, lowest_prices, websocket_prices, high_low_data, last_notified_prices):
    try:
        ...
        if symbol_status is None:
            # ตรวจสอบและอัปเดตจุดต่ำสุด (Low) ถ้าพบราคาปัจจุบันต่ำกว่า low เดิม
            if symbol not in lowest_prices or current_price < lowest_prices[symbol]:
                ...
                high_low_data[symbol]['low'] = lowest_prices[symbol]
                save_high_low(high_low_data)  # บันทึกค่า low ที่อัปเดตลงไฟล์
                cmd_notify(f"ราคาต่ำสุดใหม่ของ {symbol}: {lowest_prices[symbol]}")

            # ตั้งเป้าหมายการซื้อใหม่จากจุดต่ำสุด
...
def monitor_and_trade_multiple_pairs(pairs, buy_usdt_amount):
    line_notify("เริ่มรัน Robot: ระบบพร้อมแล้ว!")
