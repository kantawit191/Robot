...
# กำหนดค่า drop_percentage และ rise_percentage ของแต่ละคู่เงิน
drop_percentage_dict = {
    'BTCUSDT': 0.8,
    'ETHUSDT': 1.0,
    'ADAUSDT': 0.7,
    # เพิ่มคู่เงินและค่าที่ต้องการได้ที่นี่
}

rise_percentage_dict = {
    'BTCUSDT': 2.5,
    'ETHUSDT': 2.8,
    'ADAUSDT': 3.0,
    # เพิ่มคู่เงินและค่าที่ต้องการได้ที่นี่
...
def place_buy_order(symbol, usdt_amount, price, trade_status, trade_status2, file_name ,buy_number=1):
    global latest_balance
    drop_percentage = drop_percentage_dict[symbol]
    fee_percentage = 0.1
    with threading.Lock():
        try:
...
def place_sell_order(symbol, trade_status, rise_percentage,trade_status1, trade_status2,websocket_prices, file_name, buy_number=1):
    try:
        rise_percentage = rise_percentage_dict[symbol]
...
symbols = ['BTCUSDT', 'ETHUSDT', 'ADAUSDT']  # ตัวอย่างคู่เงิน
buy_usdt_amount = 180
monitor_and_trade_multiple_pairs(symbols, drop_percentage_dict, rise_percentage_dict, buy_usdt_amount)
